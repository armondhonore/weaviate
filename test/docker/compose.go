...
func (d *Compose) Start(ctx context.Context) (*DockerCompose, error) {
	...
	if d.withSecondWeaviate {
		...
		container, err := startWeaviate(ctx, d.enableModules, d.defaultVectorizerModule, envSettings, networkName, d.netOctet, image, hostname, d.withWeaviateExposeGRPCPort, d.withWeaviateExposeDebugPort, d.withWeaviateTmpfsData, "/v1/.well-known/ready", d.weaviateFiles)
		if err != nil {
	...
	startNodeWithRetry := func(cfg map[string]string, hostname string) (*DockerContainer, error) {
		...
		for attempt := 0; attempt <= maxRetries; attempt++ {
			...
			c, err := startWeaviate(attemptCtx, d.enableModules, d.defaultVectorizerModule,
				cfg, networkName, d.netOctet, image, hostname, d.withWeaviateExposeGRPCPort, d.withWeaviateExposeDebugPort, d.withWeaviateTmpfsData, livenessEndpoint, d.weaviateFiles)
			cancel()
			if err == nil {
				if attempt > 0 {
