...
// maxCleanupAttempts caps per-segment rewrite retries before the segment is
// quarantined (C6), so a permanently-failing segment can't wedge a drop op in
...
func (c *segmentCleanerCommon) cleanupOnceEditOps(shouldAbort cyclemanager.ShouldAbortCallback,
	...
	transformer, err := c.sg.buildValueTransformer()
	if err != nil {
...
// cleanPendingSegmentToTmp locates the segment by id in the current view and
// rewrites it (minus keys shadowed by newer segments, with the transformer
// applied) into a .tmp file. found is false when the segment is no longer on
// disk. Mirrors cleanupOnce: the returned idx is consumed by replaceSegment.
