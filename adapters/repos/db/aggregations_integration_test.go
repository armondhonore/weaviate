...
func prepareCompanyTestSchemaAndData(repo *DB,
	...
	return func(t *testing.T) {
		...
		t.Run("creating the class", func(t *testing.T) {
			...
			require.Nil(t,
				migrator.AddClass(context.Background(), customerClass))
			require.Nil(t,
				migrator.AddClass(context.Background(), notIndexedClass))
		})
			...
			for j := 0; j < importFactor; j++ {
				for i, schema := range companies {
					t.Run(fmt.Sprintf("importing company %d", i), func(t *testing.T) {
						...
						require.Nil(t,
							repo.PutObject(context.Background(), &fixture, []float32{0.1, 0.1, 0.1, 0.1}, nil, nil, nil, 0))
					})
				}
			}
		})
